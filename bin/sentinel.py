...
def perform_argod_object_sync(argod):
    GovernanceObject.sync(argod)


# delete old watchdog objects, create new when necessary
def watchdog_check(argod):
    printdbg("in watchdog_check")

    # delete expired watchdogs
    for wd in Watchdog.expired(argod):
        printdbg("\tFound expired watchdog [%s], voting to delete" % wd.object_hash)
        wd.vote(argod, VoteSignals.delete, VoteOutcomes.yes)

    # now, get all the active ones...
    active_wd = Watchdog.active(argod)
    ...
    if 0 == active_count:
        ...
        winner.vote(argod, VoteSignals.valid, VoteOutcomes.yes)
...
def prune_expired_proposals(argod):
    # vote delete for old proposals
    for proposal in Proposal.expired(argod.superblockcycle()):
        proposal.vote(argod, VoteSignals.delete, VoteOutcomes.yes)


# ping argod
def sentinel_ping(argod):
...
def attempt_superblock_creation(argod):
    import argolib
    ...
    event_block_height = argod.next_superblock_height()
    ...
    if Superblock.is_voted_funding(event_block_height):
        ...
        for sb in Superblock.at_height(event_block_height):
            if not sb.voted_on(signal=VoteSignals.funding):
                sb.vote(argod, VoteSignals.funding, VoteOutcomes.no)

        # now return, we're done
        return

    if not argod.is_govobj_maturity_phase():
    ...
    sb = argolib.create_superblock(proposals, event_block_height, budget_max, sb_epoch_time)
    if not sb:
    ...
    if dbrec:
        dbrec.vote(argod, VoteSignals.funding, VoteOutcomes.yes)

        # any other blocks which match the sb_hash are duplicates, delete them
        for sb in Superblock.select().where(Superblock.sb_hash == sb.hex_hash()):
            if not sb.voted_on(signal=VoteSignals.funding):
                sb.vote(argod, VoteSignals.delete, VoteOutcomes.yes)
    ...
    if (argod.we_are_the_winner()):
        printdbg("we are the winner! Submit SB to network")
        sb.submit(argod)


def check_object_validity(argod):
    # vote (in)valid objects
    for gov_class in [Proposal, Superblock]:
        for obj in gov_class.select():
            obj.vote_validity(argod)


def is_argod_port_open(argod):
    # test socket open before beginning, display instructive message to MN
    # operators if it's not
    port_open = False
    try:
        info = argod.rpc_command('getgovernanceinfo')
        port_open = True
