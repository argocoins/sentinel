...
def test_valid_argo_address():
    from argolib import is_valid_argo_address

    main = valid_argo_address()
    test = valid_argo_address('testnet')

    assert is_valid_argo_address(main) is True
    assert is_valid_argo_address(main, 'mainnet') is True
    assert is_valid_argo_address(main, 'testnet') is False

    assert is_valid_argo_address(test) is False
    assert is_valid_argo_address(test, 'mainnet') is False
    assert is_valid_argo_address(test, 'testnet') is True


def test_invalid_argo_address():
    from argolib import is_valid_argo_address

    main = invalid_argo_address()
    test = invalid_argo_address('testnet')

    assert is_valid_argo_address(main) is False
    assert is_valid_argo_address(main, 'mainnet') is False
    assert is_valid_argo_address(main, 'testnet') is False

    assert is_valid_argo_address(test) is False
    assert is_valid_argo_address(test, 'mainnet') is False
    assert is_valid_argo_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from argolib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % argolib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(argolib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(argolib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(argolib.blocks_to_seconds(16616)) == 2612035
