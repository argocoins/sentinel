...
def is_valid_argo_address(address, network='mainnet'):
    ...
    argo_version = 140 if network == 'testnet' else 23
    ...
    try:
        ...
        # rescue from exception, not a valid Argo address
        return False

    if (address_version != argo_version):
        return False
...
# shims 'til we can fix the argod side
def SHIM_serialise_for_argod(sentinel_hex):
    ...
    # superblock => "trigger" in argod
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # argod expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    argod_hex = serialise(obj)
    return argod_hex


# shims 'til we can fix the argod side
def SHIM_deserialise_from_argod(argod_hex):
    from models import ARGOD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(argod_hex)
