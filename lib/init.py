...
def has_argo_conf():
    ...
    valid_argo_conf = False

    # ensure argo_conf exists & readable
    #
    # if not, print a message stating that Argo Core must be installed and
    # configured, including JSONRPC access in argo.conf
    try:
        f = io.open(config.argo_conf)
        valid_argo_conf = True
    except IOError as e:
        print(e)

    return valid_argo_conf
...
def main():
    ...
    if not has_argo_conf():
        print("ArgoCore must be installed and configured, including JSONRPC access in argo.conf")
        sys.exit(1)
