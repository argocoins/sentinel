...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, argod):
        golist = argod.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_argod(argod, item)
    ...
    @classmethod
    def import_gobject_from_argod(self, argod, rec):
        import decimal
        import argolib
        import inflection
        ...
        # shim/argod conversion
        object_hex = argolib.SHIM_deserialise_from_argod(object_hex)
        objects = argolib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from argod...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from argod, with every run
        ...
        # get/create, then sync payment amounts, etc. from argod - Argod is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(argod)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from argod! %s" % e)
            govobj.vote_delete(argod)
            return (govobj, None)
    ...
    def vote_delete(self, argod):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(argod, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, argod, signal, outcome):
        import argolib
        ...
        output = argod.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = argolib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(argod, signal)

    def sync_network_vote(self, argod, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = argod.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 argo addr, non-multisig
            if not argolib.is_valid_argo_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Argo address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (argolib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/ArgoDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import argolib
        obj_data = argolib.SHIM_serialise_for_argod(self.serialise())
    ...
    def prepare(self, argod):
        try:
            object_hash = argod.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not argolib.is_valid_argo_address(addr, config.network):
    ...
    def hash(self):
        import argolib
        return argolib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, argod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - argod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, argod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - argod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, argod):
        now = int(time.time())
        return (self.created_at < (now - argod.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, argod):
        if self.is_expired(argod):
            return False

        return True

    def is_deletable(self, argod):
        if self.is_expired(argod):
            return True
